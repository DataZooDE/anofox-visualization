...
wasm-opt -Oz --strip-debug target/wasm32-unknown-emscripten/debug/duckplot_duckext.wasm -o /tmp/ggext.wasm

echo "== package (duckdb_signature custom section) =="
REPO="$GG/web/extrepo/v1.1.1/wasm_eh"; mkdir -p "$REPO"
python3 "$HERE/scripts/append_extension_metadata.py" -l /tmp/ggext.wasm -n ggplot \
  -o "$REPO/ggplot.duckdb_extension.wasm" -p wasm_eh -dv v0.0.1 -ev v0.1.0 >/dev/null
python3 -c "import gzip;d=open('$REPO/ggplot.duckdb_extension.wasm','rb').read();open('$REPO/ggplot.duckdb_extension.wasm.gz','wb').write(gzip.compress(d))"
...
const r = await p.evaluate(async () => {
  ...
  try {
    ...
    await c.query("INSTALL ggplot;"); await c.query("LOAD ggplot;"); s.load = "ok";
    s.result = (await c.query("SELECT length(ggplot_smoke()) AS n")).toArray().map(r=>r.toJSON());
